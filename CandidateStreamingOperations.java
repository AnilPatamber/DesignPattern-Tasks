...
public class CandidateStreamingOperations {
	...
	public static void main(String[] args) {
		...
		Map<String,Long> expertiseCandid = candidatesList.stream()
											.map(candid->candid.getTechnicalExpertise())
											.collect(Collectors.groupingBy(Function.identity(),Collectors.counting()));
		System.out.println("\nCandidate count with technical expertise :\n "+expertiseCandid);
		
		//Task 4 : list fresher candidates
		List<Candidate> freshersCandid = candidatesList.stream()
											.filter(candid->(candid.getYearsOfExperience()==0))
											.collect(Collectors.toList());
